...
5. Build a local Markdown render pipeline that converts document text into rendered preview HTML and supports Mermaid fenced blocks through a local rendering path.
6. Configure the webview with a strict CSP and the minimum required capabilities, expanding only as needed for strictly local Mermaid rendering.
...
9. Show an in-viewer error state on document render failure and local fallback markup on Mermaid block failure instead of falling back to the default Markdown editor.
...
5. Open a Markdown document that contains a Mermaid fenced block and confirm the diagram renders inside the viewer.
6. Open a Markdown document with invalid Mermaid syntax and confirm the user remains in the Inlinr viewer, the rest of the document still renders, and the Mermaid block shows a clear local fallback.
7. Open a Markdown document that contains a null character from disk, or rely on the automated error-state tests, and confirm the user remains in the Inlinr viewer with a clear error state.
8. Confirm no document content is sent to external services when opening or refreshing the viewer, including Mermaid-capable documents.

## Test Targets

- Unit tests for Markdown rendering helpers, Mermaid rendering and fallback handling, and document-session coordination.
- Integration tests for provider resolution, custom editor state generation, Mermaid viewer output, non-Markdown fallback, refresh behavior, and in-viewer error handling.
- Manual verification for accessibility, theme compatibility, Mermaid behavior, and the main open flows.

## Notes

- Do not add provider or AI request plumbing in this slice.
- Keep the webview as capability-minimal as possible while still supporting the chosen local Mermaid rendering path.
