...
1. Emit stable DOM attributes for source-backed selectable regions and markers during Markdown rendering.
2. Enable the minimum webview scripting needed to resolve the actual rendered DOM selection and manage the overlay controller.
3. Define the narrowed webview-to-extension message contract for selection capture, cancel, and submit-only draft handoff.
...
6. Keep popup placement, focus, scrolling, and draft entry in a client-owned overlay inside the webview.
7. Create a provider-agnostic local request payload only on explicit submit.
...
4. Scroll the document with the popup open and confirm the overlay remains visually attached to the selected area rather than drifting to the end of the document.
5. Select a larger contiguous rendered prose range across adjacent prose blocks and confirm the popup still targets one selection.
6. Try selecting across a heading boundary, code fence, image, or unsupported region and confirm the product blocks the request and asks for a supported re-selection.
7. Open one draft, then try to start another on a different selection and confirm the first draft must be canceled or finished before a new one opens.
8. Change the document after opening a draft, then submit and confirm the system either revalidates the target or blocks submit with a clear reselect path.
9. Submit a valid request and confirm the product creates the local request payload without routing through another extension's chat textbox.
10. Confirm no full-document payload is created by default and no external request occurs before explicit submit.
...
- DOM-driven regression coverage for real rendered selection behavior, popup visibility near the selection, and scroll-safe overlay positioning.
...
- Keep popup layout and draft-entry responsiveness in the webview rather than round-tripping every transient change through host rerenders.
