...
## Decision 4: Ship a bundled default Application Insights connection string, allow environment override, and fail open for editing when telemetry is unavailable

- Decision: Bundle a default Application Insights connection string in `src/telemetry/defaultTelemetryConfig.ts` so telemetry works in all shipped builds without any configuration. Allow the connection string to be overridden via `APPLICATIONINSIGHTS_CONNECTION_STRING` in the environment. Never let telemetry misconfiguration block normal editing behavior.
- Rationale: Without a bundled default, real marketplace installs never had the env var set and the sink always fell back to no-op — telemetry was silently disabled for all users. The Application Insights connection string is a public ingestion endpoint, not a credential (the security model is that the endpoint is publicly known and the data it receives is telemetry, not user secrets). Committing it in source is the standard pattern for open-source VS Code extensions. Inlinr's monitoring feature is operationally valuable, but it must not become a runtime dependency for the editing loop. If the Azure sink cannot initialize, the adapter falls back to a no-op sink.
- Alternatives considered: Environment-only connection strings were tried first but silently disabled telemetry for all real users since the env var was never set in the extension host. Failing the extension or monitored scenarios when telemetry is unavailable was rejected because monitoring must not reduce product availability.
